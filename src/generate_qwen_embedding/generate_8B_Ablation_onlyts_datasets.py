...
"""
从 ver_8B_Ablation_onlyts 数据集生成三个新数据集：
1. ver_8B_Ablation_onlyts_trendstrength - 包含趋势和强度信息
2. ver_8B_Ablation_onlyts_trendonly - 只包含趋势信息
3. ver_8B_Ablation_onlyts_globalonly - 只包含全局趋势信息
...
def main():
    ...
    input_dataset = "ver_8B_Ablation_onlyts"
    input_dir = os.path.join(script_dir, base_dir, input_dataset)
    
    output_datasets = [
        ("ver_8B_Ablation_onlyts_trendstrength", generate_trendstrength_dataset),
        ("ver_8B_Ablation_onlyts_trendonly", generate_trendonly_dataset),
        ("ver_8B_Ablation_onlyts_globalonly", generate_globalonly_dataset)
    ]
