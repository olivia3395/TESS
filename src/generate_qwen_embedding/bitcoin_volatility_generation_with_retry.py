...
"""Generate volatility-formatted texts with retry and extraction for Bitcoin dataset.for Bitcoin dataset."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate volatility-formatted texts with retry for Bitcoin dataset"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default="/home/llh/MMTSF/MMTSF_LIB/dataset/Bitcoin/ver_volatility",
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "You are a professional cryptocurrency analyst specializing in Bitcoin. Analyze the 7-step Bitcoin price sequence "
            "{{{historical_data}}} with mean value {mean_value}.\n\n"
            "Step 1: Calculate percentage changes between consecutive steps\n"
            "Step 2: Determine volatility level based on these criteria:\n"
            "- High Volatility: Significant fluctuations (>7% between steps)\n"
            "- Medium Volatility: Moderate fluctuations (3-7% between steps)\n"
            "- Low Volatility: Small fluctuations (<3% between steps)\n\n"
            "Bitcoin-specific considerations:\n"
            "- Bitcoin is inherently more volatile than traditional assets\n"
            "- Major events (halvings, regulatory news, market sentiment) can cause extreme volatility\n"
