...
"""
带有验证和重新生成机制的改进版生成代码
针对Electricity数据集的Shape分类任务生成文本并提取相应字段
"""
...
def _process_data_on_gpu(args, data, split_name, gpu_id):
    ...
    for item in data:
        ...
        # Electricity数据集ver_camf直接包含news字段
        news = item.get('news', '')
...
def main():
    parser = argparse.ArgumentParser(description='Generate Qwen texts using improved approach with retry mechanism for Electricity dataset')
    parser.add_argument('--data-path', type=str, default='/home/llh/MMTSF/MMTSF_LIB/dataset/Electricity/ver_camf', 
                       help='Path to Electricity dataset')
    parser.add_argument('--output-path', type=str, default='/home/llh/MMTSF/MMTSF_LIB/dataset/Electricity/ver_shape', 
    ...
    # Electricity数据集的Shape任务提示模板
    parser.add_argument('--prompt-template', type=str, 
                   default="Analyze the 48-step electricity load sequence {{{historical_data}}} and news context: {news}\n\nClassify the shape into one category:\n- Rise: Overall upward trend from start to end\n- Fall: Overall downward trend from start to end\n- Peak: Rises in first half, falls in second half\n- Recover: Falls in first half, rises in second half\n- Oscillate: Multiple direction changes\n\nCRITICAL: You must output EXACTLY this format with no other text:\nShape:Rise\n\nReplace Rise with one of: Rise, Fall, Peak, Recover, or Oscillate.\n\nDO NOT output explanations, analysis, or any other text. Output ONLY the line above.",
