...
"""Generate volatility-formatted texts with retry and extraction for Electricity dataset."""
...
def load_electricity_data(data_path, split):
    """Load Electricity split."""
...
def extract_volatility_field(text: str):
    ...
    # Remove any leading/trailing content that might interfere
    ...
    alt_patterns = [
        re.compile(r"(High|Medium|Low)\s*volatility", re.IGNORECASE),
        re.compile(r"volatility\s*:\s*(High|Medium|Low)", re.IGNORECASE),
        ...
        for i in tqdm(range(0, len(prompts), batch_size), desc="Generating texts"):
            ...
            for prompt_idx, (prompt, orig_item) in enumerate(
                ...
                while retry_count < max_retries:
                    # Modify prompt on retries to be more insistent
                    current_prompt = prompt
                    if retry_count > 0:
                        current_prompt = prompt + f"\n\nIMPORTANT: This is retry #{retry_count}. You MUST output ONLY: 'Global volatility: High' or 'Global volatility: Medium' or 'Global volatility: Low'"
                    ...
                    # Use moderate temperature for better quality output
                    temp = 0.7 if retry_count == 0 else 0.3  # Moderate temp on first try, lower on retries
                    ...
                    for line in lines:
                        line = line.strip()
                        if line.startswith('Global volatility:'):
                            generated_text = line
                            break
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate volatility-formatted texts with retry for Electricity dataset"
    )
    parser.add_argument(
        "--data-path",
        type=str,
        default="/home/llh/MMTSF/MMTSF_LIB/dataset/Electricity/ver_camf",
        help="Path to Electricity dataset",
    )
    parser.add_argument(
        "--output-path",
        type=str,
        default="/home/llh/MMTSF/MMTSF_LIB/dataset/Electricity/ver_volatility",
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "Analyze this 48-step electricity consumption sequence: {{{historical_data}}} (mean: {mean_value})\n"
            "News context: {news}\n\n"
            "CLASSIFY VOLATILITY:\n"
            "- Low: Small changes (<18% between steps)\n"
            "- Medium: Moderate changes (18-28% between steps)\n"
            "- High: Large changes (>28% between steps)\n\n"
            "RULES:\n"
            "1. Calculate step-to-step percentage changes\n"
            "2. Count changes >28% (High), changes 18-28% (Medium), changes <18% (Low)\n"
            "3. If majority of changes <18%: Low\n"
            "4. If multiple changes >28%: High\n"
            "5. Otherwise: Medium\n\n"
            "OUTPUT ONLY ONE LINE:\n"
            "Global volatility: High\n"
            "Global volatility: Medium\n"
            "Global volatility: Low\n\n"
            "NO analysis, NO steps, NO explanation. Just the one output line."
        ),
    ...
    if len(gpu_ids) > 1:
        ...
        for split_name in args.splits:
            ...
            split_data = load_electricity_data(args.data_path, split_name)
            if args.test_limit is not None:
