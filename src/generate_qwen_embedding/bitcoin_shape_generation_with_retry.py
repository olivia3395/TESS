...
def main():
    parser = argparse.ArgumentParser(description='Generate Qwen texts using improved approach with retry mechanism for Bitcoin dataset')
    parser.add_argument('--data-path', type=str, default='/home/llh/MMTSF/MMTSF_LIB/dataset/Bitcoin/ver_camf', 
                       help='Path to FNSPID dataset')
    parser.add_argument('--output-path', type=str, default='/home/llh/MMTSF/MMTSF_LIB/dataset/Bitcoin/ver_shape', 
    ...
    parser.add_argument('--prompt-template', type=str, 
                   default="You are a professional cryptocurrency analyst specializing in Bitcoin. Analyze the 7-step Bitcoin price sequence {{{historical_data}}} and related news {news} to determine the overall shape pattern.\n\nClassify the shape into one of five categories:\n- Rise: Generally upward path with no meaningful pullbacks\n- Fall: Generally downward path with no meaningful rebounds\n- Peak: Early steps rise then later steps fall, with ONLY ONE turning point\n- Recover: Early steps fall then later steps rise, with ONLY ONE turning point\n- Oscillate: Up/down mixed movements with TWO OR MORE turning points\n\nBitcoin-specific considerations:\n- Bitcoin often shows more extreme patterns than traditional assets\n- Halving events and regulatory news can cause significant directional changes\n- Market sentiment and macroeconomic factors strongly influence short-term movements\n\nOutput ONLY in this exact format with no additional explanations, analysis, or context:\nShape:<Rise or Fall or Peak or Recover or Oscillate>\n\nIMPORTANT: ONLY output the specified format above with no other content before or after.",
