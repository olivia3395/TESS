...
class meta_ablation(nn.Module):
    def __init__(self, configs):
        super(meta_ablation, self).__init__()
    ...
    def forward(self,x_enc): # 后续要添加test_flag参数
        """
        时序编码->解耦  获得时序的初始表征 
        """
        """
        ...
        env, entity = self.temporal(enc_out) 
        
        # ### Static Metadata Knowledge Learning 
        # #### VQ VAE 
        
        flatten_h_env = env.view(self.batch_size,-1)
        ...
        self.h_final_repr = h_env


        
        ### Decoder 
        dec_out = self.decoder_mlp(self.h_final_repr).unsqueeze(-1)
