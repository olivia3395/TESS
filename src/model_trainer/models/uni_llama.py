...
class uni_llama(nn.Module):

    def __init__(self, config):
        super(uni_llama, self).__init__()
        ...
        if self.llm_model_type == 'llama':
            self.local_path = config["llama_7b_path"]
            self.llama_config = LlamaConfig.from_pretrained(self.local_path)
    ...
    def forward(self, x_enc,x_text,flag):
    ...
    def calculate_loss(self,batch_y):
        loss_fuc = nn.MSELoss()
        return loss_fuc(self.dec_out,batch_y)
