...
class CaDisen_ct(nn.Module):
    def __init__(self, configs):
        super(CaDisen_ct, self).__init__()
    ...
    def forward(self,x_enc,meta_feat,news_feat,ct1_feat,flag='train'):
        """
        时序编码->解耦  获得时序的初始表征 
        """
        """
        ...
        # ct_embed = self.dynamic_fc(ct1_feat)
        # h_entity_news = self.dynamic_fused_layer(h_entity,news_embed)
        # h_entity_ct1 = self.dynamic_fused_layer(h_entity,ct_embed)
        # h_entity = self.alpha * h_entity_news + (1 - self.alpha) * h_entity_ct1

        # 反事实代替news
        ct_embed = self.dynamic_fc(ct1_feat)
        h_entity = self.dynamic_fused_layer(h_entity,ct_embed)
