...
"""
Dynamic Dropout MultiModal Baseline with Max Pooling:

基于MultiModal_Baseline_Dynamic_Dropout，将文本embedding的池化方式从平均池化改为最大池化：
...
5. 使用最大池化代替平均池化来聚合token embeddings

实验目标：探究文本中哪些token对时间序列预测最重要，通过学习性dropout机制实现。
使用最大池化可以突出最重要的token特征。
"""

class MultiModal_Baseline_Dynamic_Dropout_MaxPool(nn.Module):
    def __init__(self, configs):
        super(MultiModal_Baseline_Dynamic_Dropout_MaxPool, self).__init__()
    ...
    def forward(self,x_enc,news_feat,flag='train', attention_mask=None):
        ...
        # Pool to get sentence-level representation [B, D] using MAX POOLING
        # Use attention_mask to mask out padding tokens for proper max pooling
        if attention_mask is not None:
            # Mask out padding tokens by setting them to a very small value
            attention_mask_expanded = attention_mask.unsqueeze(-1).float()  # [B, L, 1]
            # Set padding tokens to a very negative value so they won't be selected by max pooling
            masked_embeddings = dropout_word_embeddings * attention_mask_expanded + (1 - attention_mask_expanded) * (-1e9)
            h_text = masked_embeddings.max(dim=1)[0]  # [B, D]
        else:
            h_text = dropout_word_embeddings.max(dim=1)[0]  # [B, D]
