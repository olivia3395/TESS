#!/usr/bin/env python
"""绘制两个MultiModal_Baseline模型的loss曲线"""
...
def plot_loss_curves():
    ...
    manifest_files = {
        'ver_global_shape_volatility_natural': base_dir / 'output' / 'FNSPID' / 'ver_global_shape_volatility_natural' / 'MultiModal_Baseline' / 'Jan-20-2026-02-20-54' / 'manifest.json',
        'ver_camf': base_dir / 'output' / 'FNSPID' / 'ver_camf' / 'MultiModal_Baseline' / 'Jan-20-2026-01-26-09' / 'manifest.json'
    }

    # 颜色设置
    colors = {
        'ver_global_shape_volatility_natural': '#00008B',  # 深蓝色
        'ver_camf': '#006400'  # 深绿色
    }

    # 标签设置
    labels = {
        'ver_global_shape_volatility_natural': 'Ours',
    ...
    plt.title('FNSPID - Training Loss Curves')
    plt.legend(loc='best')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    # 保存训练loss图
    train_output = output_dir / 'training_loss_comparison.png'
    ...
    plt.title('FNSPID - Validation Loss Curves')
    plt.legend(loc='best')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    # 保存验证loss图
    vali_output = output_dir / 'validation_loss_comparison.png'
