#!/usr/bin/env python
"""绘制Environment数据集两个MultiModal_Baseline模型的loss曲线"""
...
def plot_loss_curves():
    ...
    manifest_files = {
        'ver_global_temporal_shape_volatility_natural': base_dir / 'output' / 'Environment' / 'ver_global_temporal_shape_volatility_natural' / 'MultiModal_Baseline' / 'Jan-20-2026-03-49-16' / 'manifest.json',
        'ver_camf': base_dir / 'output' / 'Environment' / 'ver_camf' / 'MultiModal_Baseline' / 'Jan-20-2026-03-47-02' / 'manifest.json'
    }
    ...
    plt.title('Environment - Training Loss Curves')
    plt.legend(loc='best')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    # 保存训练loss图
    train_output = output_dir / 'environment_training_loss_comparison.png'
    ...
    plt.title('Environment - Validation Loss Curves')
    plt.legend(loc='best')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    # 保存验证loss图
    vali_output = output_dir / 'environment_validation_loss_comparison.png'
    plt.savefig(vali_output, dpi=300, bbox_inches='tight')
    print(f"Saved validation loss plot to: {vali_output}")
    plt.close()

    print("\nEnvironment数据集Loss曲线绘制完成！")
