...
# generate_bitcoin_token_embeddings.sh - 为Bitcoin数据集生成token embedding
...
"ver_volatility_temporal"
"ver_volatility_shape"
"ver_temporal_shape_shape"
...
# 日志文件
LOG_FILE="bitcoin_token_embedding_generation.log"

# 创建日志目录
mkdir -p logs

echo "开始为以下Bitcoin数据集生成token embedding:" | tee "$LOG_FILE"
printf '%s\n' "${DATASETS[@]}" | tee -a "$LOG_FILE"
echo "基础目录: $BASE_DIR" | tee -a "$LOG_FILE"
echo "字段名: $FIELD_NAME" | tee -a "$LOG_FILE"
echo "模型路径: 使用Python脚本默认路径 (/ssd/hf_home/models/Qwen3-8B)" | tee -a "$LOG_FILE"
echo "========================" | tee -a "$LOG_FILE"
...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    # 执行命令并将输出保存到单独的日志文件
    DATASET_LOG="logs/bitcoin_token_$(echo $DATASET_NAME | tr '/' '_').log"
